...
export const registerUser = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    return next(new AppError('User creation failed: ' + err.message, 400));
  }
...
export const loginUser = asyncHandler(async (req, res, next) => {
  ...
  // ✅ Step 1: Check the role and set the message variable
  const message = user.role === 'ADMIN' 
    ? 'Admin logged in successfully' 
    : 'User logged in successfully';

  // ✅ Step 2: Use the 'message' variable in the response JSON
  res.status(200).json({
    success: true,
    message: message, // Use the variable here instead of a hardcoded string
    user,
  });
});
